...
pub(crate) fn expand_peripheral_input_device_config(
    ...
    let (adc_devices, _adc_processors) = match &board {
        // Split peripherals do not run input processors in RMK 0.8.x, so a
        // battery ADC configured in [ble] would produce raw battery events that
        // are forwarded to the central and interpreted as central battery
        // level. Keep joystick ADCs, but skip the global battery ADC here.
